...
def create():
    ...
    if (
        (timestamp is None)
        or (hmac_sign is None)
        or (
            not check_HMAC(
                timestamp, [email, password, name, surname, patronymic], hmac_sign
            )
        )
    ):
...
def delete():
    ...
    if (
        (timestamp is None)
        or (hmac_sign is None)
        or (not check_HMAC(timestamp, [email], hmac_sign))
    ):
        return ResponseData.Get_HMAC_Error()
...
def getMessages():
    ...
    if (
        (timestamp is None)
        or (hmac_sign is None)
        or (not check_HMAC(timestamp, data, hmac_sign))
    ):
        return ResponseData.Get_HMAC_Error()
...
def getPreauthLink():
    ...
    result = Zimbra.GetPreauthLink(email).asdict()
    return result
